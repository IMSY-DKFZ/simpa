...
class SpacingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.normalization == "min_max":
            high_res_data, high_res_max, high_res_min = normalize_tensor_min_max(high_res_data)
            low_res_data, low_res_max, low_res_min = normalize_tensor_min_max(low_res_data)

        elif self.normalization == "standardize":
            high_res_data, high_res_mean, high_res_std = standardize_tensor(high_res_data)
            low_res_data, low_res_mean, low_res_std = standardize_tensor(low_res_data)
...
class UpsampleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.normalization == "min_max":
            data, low_res_max, low_res_min = normalize_tensor_min_max(data, log=log)

        elif self.normalization == "standardize":
            data, low_res_mean, low_res_std = standardize_tensor(data, log=log)
...
class InitialPressureDatasetSRCNN(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        initial_pressure, init_max, init_min = normalize_tensor_min_max(initial_pressure)
...
def normalize_tensor_min_max(data, log=True):
    if log is True:
        data = np.log10(data)
    mx = np.amax(data)
    mn = np.amin(data)
...
def normalize_tensor_min_max_inverse(data, mx=1, mn=0, log=True):
    if log is True:
        data = 1 - data
        data *= -(mx - mn)
        data += mx
...
def standardize_tensor(data, log=True):
    if log is True:
        data = np.log10(data)
    mean = np.mean(data)
    std = np.std(data)
...
def standardize_tensor_inverse(data, mean=0, std=1, log=True):
    data = data * std + mean
    if log is True:
        data = 10 ** data
    return data
