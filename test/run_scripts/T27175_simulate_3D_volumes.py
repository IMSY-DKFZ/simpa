...
settings_dict["bakground_options"] = 3
...
settings_dict["cube_mua"] = [0.2, 10]
settings_dict["cube_mus"] = [50, 150]
settings_dict["cube_g"] = [0.8, 1]
settings_dict["cube_center_depth"] = [3, 10]
settings_dict["cube_center_x"] = [3, 5]
settings_dict["cube_center_y"] = [0, 1]
...
def create_random_tube(settings_dict):
    ...
    if settings_dict["randomness"]  == True: 
        ...
        tube_dict[Tags.STRUCTURE_CENTER_Z_MM] = randomize(tube_dict[Tags.STRUCTURE_CENTER_DEPTH_MIN_MM], tube_dict[Tags.STRUCTURE_CENTER_DEPTH_MAX_MM])
...
def create_random_structures(settings_dict):
   ...
   #structures_dict["background"] = create_background(settings_dict)
    ...
    #for i in range(2):
     #   structures_dict["tube_" + str(i).zfill(2)] = create_random_tube(settings_dict)
    for i in range(1):
        structures_dict["cube"] = create_cube(settings_dict)
    return structures_dict

seed_index = 0
'some config stuff'
volume_info_path = "/workplace/data/3D/20200324_optical_prop_rough",
pickle_out = open(volume_info_path[0] + '/' + "settings_dict.pkl","wb")
...
while seed_index < 1000:
    ...
    settings = {
        ...
        Tags.STRUCTURES: create_random_structures(pd.read_pickle(volume_info_path[0] + '/' + "settings_dict.pkl"))
        }

    'to get all the structure info and values, check out the volume_info_path'
    pickle_out = open(settings[Tags.VOLUME_INFO_PATH] + '/'+ settings[Tags.VOLUME_NAME] + "_structure_dict.pkl","wb")
