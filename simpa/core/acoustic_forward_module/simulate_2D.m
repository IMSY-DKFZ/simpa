...
for ind = 1:num_elements
  ...
  karray.addRectElement([x, y], 0.00001, 0.00001, [angles(1, ind)]);
end
...
input_args = {'DataCast', datacast};
% 'PMLInside', settings.pml_inside, ...
...
disp(kgrid)
disp(medium)
disp(mean(medium.sound_speed, 'all'))
disp(mean(medium.density, 'all'))
disp(mean(medium.alpha_coeff, 'all'))

%figure;
%imagesc(kgrid.y_vec, kgrid.x_vec, source.p0);
%axis image;
...
%    y = elem_pos(1, ind);
%    alpha = angles(1, ind);
%    plot([x],[y], 'x', 'Color', 'b', 'LineWidth', 1);
%end
