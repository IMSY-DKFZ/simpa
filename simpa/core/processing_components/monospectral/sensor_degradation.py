...
class SensorDegradation(ProcessingComponent):
    ...
    Component Settings::
       Tags.DEGRADATED_SENSORS: (default: 3)
    ...
    def run(self, device) -> None:
        ...
        time_series_data[degradated_sensors,:] *= (1-degradation_factors)[:, None]
        ...
        if Tags.SHIFT_TS in self.component_settings.keys():
            if isinstance(self.component_settings[Tags.SHIFT_TS], (int, float)):
                time_series_data[degradated_sensors,:] += self.component_settings[Tags.SHIFT_TS]
            else:
                time_series_data[degradated_sensors,:] += np.asarray(self.component_settings[Tags.SHIFT_TS])[:, None]
    ...
    def get_indices_and_factors(self, n_sensors: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if Tags.DEGRADATED_SENSORS in self.component_settings.keys():
            ...
            if Tags.DEGRADATION_FACTORS in self.component_settings.keys() and \
                ...
                degradation_factors *= DEFAULT_FACTOR          
        else:
            ...
            if Tags.DEGRADATION_FACTORS in self.component_settings.keys():
                if isinstance(self.component_settings[Tags.DEGRADATION_FACTORS], (list, np.ndarray)):
                    ...
                    self.logger.debug(f"Use 3 randomly chosen sensors to be degradated \
                        with factor {self.component_settings[Tags.DEGRADATION_FACTORS]}.")
                    degradated_sensors = np.random.choice(n_sensors, size=3)
                    degradation_factors = np.ones(3)*self.component_settings[Tags.DEGRADATION_FACTORS]
            else:
                self.logger.debug("Use 3 randomly chosen sensors to be broken by default.")
                degradated_sensors = np.random.choice(n_sensors, size=3)
                degradation_factors = np.ones(3)*DEFAULT_FACTOR
