...
def test_if_optical_parameter_spectra_are_provided_correct_tissue_definition_is_returned_from_generic_tissue():
    ...
    n_sample = np.linspace(1.4, 0.8, wavelengths_sample.shape[0])
    ...
    n_spectrum = Spectrum("n", wavelengths_sample, n_sample)

    actual_tissue = TISSUE_LIBRARY.generic_tissue(mua_spectrum, mus_spectrum, g_spectrum, n_spectrum)
...
def test_if_generic_tissue_is_called_with_invalid_arguments_error_is_raised():
    ...
    with pytest.raises(AssertionError):
        TISSUE_LIBRARY.generic_tissue(None, mus_spectrum, g_spectrum, n_spectrum)

    with pytest.raises(AssertionError):
        TISSUE_LIBRARY.generic_tissue(mua_spectrum, None, g_spectrum, n_spectrum)

    with pytest.raises(AssertionError):
        TISSUE_LIBRARY.generic_tissue(mua_spectrum, mus_spectrum, None, n_spectrum)

    with pytest.raises(AssertionError):
        TISSUE_LIBRARY.generic_tissue(mua_spectrum, mus_spectrum, g_spectrum, None)
...
def test_if_optical_parameter_spectra_are_provided_correct_tissue_definition_is_returned_from_constant():
    ...
    actual_tissue = TISSUE_LIBRARY.constant(mua_sample, mus_sample, g_sample, n_sample)
    assert actual_tissue.segmentation_type == SegmentationClasses.GENERIC

    assert len(actual_tissue) == 1
    molecule: Molecule = actual_tissue[0]
    assert molecule.volume_fraction == 1.0
    assert (molecule.absorption_spectrum.values == mua_sample).all()
    assert (molecule.scattering_spectrum.values == mus_sample).all()
    assert (molecule.anisotropy_spectrum.values == g_sample).all()
    assert (molecule.refractive_index.values == n_sample).all()
...
def test_if_constant_is_called_with_invalid_arguments_error_is_raised():
    ...
    with pytest.raises(TypeError):
        TISSUE_LIBRARY.constant(None, mus_sample, g_sample, n_sample)

    with pytest.raises(TypeError):
        TISSUE_LIBRARY.constant(mua_sample, None, g_sample, n_sample)

    with pytest.raises(TypeError):
        TISSUE_LIBRARY.constant(mua_sample, mus_sample, None, n_sample)

    with pytest.raises(TypeError):
        TISSUE_LIBRARY.constant(mua_sample, mus_sample, g_sample, None)
