...
def create_measurement_setup(sample_tickness_mm):
    ...
    background_dictionary[Tags.MOLECULE_COMPOSITION] = TISSUE_LIBRARY.constant(1e-30, 1e-30, 1.0)
    ...
    air_tube[Tags.MOLECULE_COMPOSITION] = TISSUE_LIBRARY.constant(1e-30, 1e-30, 1.0)
...
for wavelength in settings[Tags.WAVELENGTHS]:
    ...
    fluence_end = fluence[:, :, end_z+1]

    total_reflection = np.sum(fluence_start) / incident_energy
    total_transmission = np.sum(fluence_end) / incident_energy
...
print("TRANSMITTANCE RATIO:", transmittance_ratio)
print("REFLECTANCE RATIO:", reflectance_ratio)
