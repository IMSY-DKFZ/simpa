...
def get_device():
    ...
    # pa_device = PhotoacousticDevice(device_position_mm=np.asarray([45, 20, 20]),
    #                                 field_of_view_extent_mm=np.asarray([-20, 20, 0, 0, 0, 40]))
    # pa_device.set_detection_geometry(LinearArrayDetectionGeometry(device_position_mm=np.asarray([45, 20, 20]),
    #                                  field_of_view_extent_mm=np.asarray([-20, 20, 0, 0, 0, 40])))
    # pa_device.add_illumination_geometry(SlitIlluminationGeometry(slit_vector_mm=[30, 0, 0]))

    # pa_device = PhotoacousticDevice(device_position_mm=np.asarray([45, 20, 45]))
    # pa_device.set_detection_geometry(Random3DArrayDetectionGeometry(device_position_mm=np.asarray([45, 20, 45]),
    #                                                                 seed=4711,
    #                                                                 extent_mm=20))
    # pa_device.field_of_view_extent_mm = pa_device.detection_geometry.field_of_view_extent_mm
    # pa_device.add_illumination_geometry(SlitIlluminationGeometry(slit_vector_mm=[30, 0, 0]))

    # pa_device = MSOTAcuityEcho(device_position_mm=np.asarray([45, 20, 10]))
